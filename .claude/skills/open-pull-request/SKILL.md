...
2. Propose PR title in Conventional Commits format: `type(scope): description` (see `.claude/rules/git.md`).
...
5. Scan the diff for `CLAUDE.md`, `.claude/rules/`, and `.claude/skills/` changes — if conventions changed but no instruction/skill file was updated, flag it before opening.
6. If `cv.json` changed, mention what visitors will see differently (e.g. "new role added", "skills card rebalanced").

## Opening the PR

```bash
gh pr create --title "docs(cv): add Avanade Manager role" --body "$(cat <<'EOF'
## 📋 Summary
<summary>
